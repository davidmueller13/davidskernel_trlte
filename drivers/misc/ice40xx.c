...
static int gpio_fpga_main_clk;
static int gpio_fpga_main_clk_func;
...
static void irled_power_onoff(int onoff)
{
	if(gpio_ir_led_en == NO_PIN_DETECTED) {
		int ret;
		static struct regulator *pwr_ldo;
		struct device dev;
		dev = g_data->client->dev;
		if (!pwr_ldo) {
			pwr_ldo = regulator_get(&dev, "vdd");
			if (pwr_ldo != NULL) {
				pr_irda("%s : regulator_voltage : %d \n", __func__, regulator_get_voltage(pwr_ldo));
				ret = regulator_set_voltage(pwr_ldo, PWR_3_3V, PWR_3_3V);
				if (IS_ERR(pwr_ldo)) {
					printk(KERN_ERR"could not get pma8084_l16, rc = %ld\n",
							PTR_ERR(pwr_ldo));
					return;
				}
			}
		...
		if (onoff) {
			...
			if (regulator_is_enabled(pwr_ldo)) {
				...
				if (ret) {
					printk(KERN_ERR"disable l16 failed, rc=%d\n",
							ret);
					return;
				}
			}
		...
		if(onoff)
			gpio_set_value(gpio_ir_led_en,GPIO_LEVEL_HIGH);
		else
			gpio_set_value(gpio_ir_led_en,GPIO_LEVEL_LOW);
}
			...
			struct irda_ice40_platform_data *pdata)
{
	...
	ret = of_property_read_u32(np, "irda_ice40,gpio_fpga_main_clk",&gpio_fpga_main_clk);
	if (ret < 0) {
		pr_err("[%s]: failed to read gpio pin for fpga main clock\n", __func__);
		return ret;
	}
	ret = of_property_read_u32(np, "irda_ice40,gpio_fpga_main_clk_func",&gpio_fpga_main_clk_func);
	if (ret < 0) {
		pr_err("[%s]: failed to read gpio alternative func number for fpga main clock\n", __func__);
		return ret;
	}
	ret = of_property_read_u32(np, "irda_ice40,ir_led_en_gpio",&gpio_ir_led_en);
	if (ret < 0) {
		pr_err("[%s]: no ir_led_en \n", __func__);
		gpio_ir_led_en = NO_PIN_DETECTED;
	}
	...
	pdata->cdone = of_get_named_gpio(np, "irda_ice40,cdone", 0);
	pdata->cresetb = of_get_named_gpio(np, "irda_ice40,cresetb", 0);

	pr_err("[%s]: clk pin : %d \n", __func__, gpio_fpga_main_clk);
	pr_err("[%s]: pin func : %d \n", __func__, gpio_fpga_main_clk_func);
	return 0;
}
#else
static int irda_ice40_parse_dt(struct device *dev,
...
static void irda_ice40_config(void)
{
	...
	/* alternative function - to use gcc_gp3_clk - func7*/
	rc = gpio_tlmm_config(GPIO_CFG(gpio_fpga_main_clk, gpio_fpga_main_clk_func,
				GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN,
				GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc)
		pr_err("%s: fpga_main_clk error : %d\n", __func__, rc);

	rc = gpio_tlmm_config(GPIO_CFG(g_pdata->cdone, 0,
			GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
			GPIO_CFG_2MA), 1);
	if (rc){
		pr_err("%s:cdone pin deprecated - Will not use cdone : %d\n", __func__, rc);
		g_pdata->cdone = NO_PIN_DETECTED;
	}
	...
	rc = gpio_tlmm_config(GPIO_CFG(g_pdata->cresetb, 0,
				...
				GPIO_CFG_2MA), 1);
	if (rc)
		pr_warning("%s: warning check cresetb pin num[%d]\n",
				__func__, rc);
	...
	rc = gpio_tlmm_config(GPIO_CFG(g_pdata->rst_n, 0,
				...
				GPIO_CFG_2MA), 1);
	if (rc)
		pr_warning("%s: warning check rst_n pin num[%d]\n",
				__func__, rc);
...
static int ice40_fpga_firmware_update_start(const u8 *data, int len)
{
	...
	do{
		...
		usleep_range(50, 70);
		udelay(50);
		if(g_pdata->cdone == NO_PIN_DETECTED)
			break;
	}while((gpio_get_value(g_pdata->cdone) != GPIO_LEVEL_HIGH) && (retry-- >= 0));
	if(g_pdata->cdone == NO_PIN_DETECTED)
		pr_irda("%s : CDONE PIN NO DETECTED. firmware update success\n", __func__);
	else if(gpio_get_value(g_pdata->cdone) == GPIO_LEVEL_LOW)
	...
	rc = gpio_tlmm_config(GPIO_CFG(g_pdata->spi_si, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
	if (rc)
		pr_err("%s: error : %d\n", __func__, rc);

	rc = gpio_tlmm_config(GPIO_CFG(g_pdata->spi_clk, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
	if (rc)
		pr_err("%s: error : %d\n", __func__, rc);
